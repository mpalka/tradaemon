...
catalogues in `tradaemon.locales`, so the viewer picks Polish or English per session.
The engine is the only writer; this only reads state.json / *.jsonl.
Run: streamlit run src/tradaemon/dashboard/app.py
"""
...
from tradaemon import __version__, config_store, i18n
...
from tradaemon.dashboard import humanize, journals, layout, price_view
from tradaemon.data.storage import TIMEFRAME_MS
from tradaemon.i18n import t
from tradaemon.research.log import load_experiments
...
def render_live() -> None:
    ...
    if _module == "portfolio":
        from tradaemon.dashboard import portfolio_view
...
def render() -> None:
    if _module == "settings":
        ...
        from tradaemon.dashboard import config_view
        config_view.render()
        ...
        from tradaemon.dashboard import research_view
