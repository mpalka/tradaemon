...
def _chart(strat: pd.DataFrame, bench: pd.DataFrame) -> alt.Chart:
    ...
    return alt.Chart(data).mark_line().encode(
        x=alt.X("timestamp:T", title=None, axis=alt.Axis(format="%m.%Y", grid=False)),
...
def _tab_allocation(state: dict, pcfg: PortfolioConfig) -> None:
    ...
    chart = alt.Chart(df).mark_bar().encode(
        ...
        yOffset="typ:N",
    ).properties(height=60 + 40 * len(pcfg.symbols))
...
def _tab_rebalances(trades: pd.DataFrame) -> None:
    ...
    cols = [c for c in ["timestamp", "symbol", "side", "qty", "price", "value", "fee", "kind"]
            if c in df]
    st.dataframe(df[cols].sort_values("timestamp", ascending=False),
...
def _tab_health(state: dict, alerts: pd.DataFrame) -> None:
    ...
    if len(alerts):
        st.dataframe(
            alerts.sort_values("timestamp", ascending=False).head(30)[
                [x for x in ["timestamp", "kind", "message"] if x in alerts]],
            use_container_width=True, hide_index=True)
...
def render() -> None:
    ...
    with st.expander("🔬 Szczegóły dla dociekliwych"):
        t = st.tabs(["Alokacja", "Rebalanse", "Zdrowie"])
        with t[0]:
            _tab_allocation(state, pcfg)
        with t[1]:
            _tab_rebalances(trades)
        with t[2]:
            _tab_health(state, alerts)
