...
*[English](howitworks.md) · Polski*

> To projekt edukacyjny działający na symulowanych pieniądzach. Patrz
> [DISCLAIMER.md](DISCLAIMER.md) oraz
> [czego się spodziewać](README.pl.md#czego-się-spodziewać-po-takim-bocie).
...
Wersja kodu, którą opisuje ten dokument: `0.2.0`.
...
`variants:` w konfiguracji to nie kilka botów. To **jeden strumień świec i kilka
niezależnych portfeli**, które dostają dokładnie te same dane w tej samej chwili:
...
Księgi progowe różnią się wyłącznie progiem, więc różnica w ich krzywych kapitału jest
różnicą progu i niczego więcej. Ostatnia ma **identyczny sygnał co `prog_050`** —
...
- `humanize.py` — liczby i zdarzenia na zwykłe zdania („kupił ETH za 100 $ — teraz +2 $")
...
- `i18n.py` / `locales/` — w jakim języku czyta oglądający. Katalogi to zwykłe słowniki;
  panel trzyma wybór na sesję, bo jeden proces obsługuje kilku widzów.
...
interakcji i przy każdym auto-odświeżeniu (60 s, bo świeca 4h zamyka się sześć razy
na dobę i częstsze pytanie kosztuje wyłącznie procesor).
...
klik / tik odświeżenia
   ...
   → st.session_state trzyma to, co wybrał człowiek
        (rozwinięty instrument, zakres 7d/30d, moduł, język)
   → render
```
...
- **Wybór musi żyć w `session_state`.** Bez tego odświeżenie zamykałoby
